...
# ANTHROPIC_API_KEY= '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'
...
#   <div style="font-size:14px;color:#888;">during the blessed last nights of Ramadan</div>
# </div>
# {msg_html}
# {dua_html}
# <div style="text-align:center;margin-top:30px;padding-top:20px;border-top:1px solid #d4c4a0;">
...
#         f"during the blessed last nights of Ramadan.\n\n"
...
#     client_ip = get_client_ip(request)
#     allowed, _ = db.rate_limit_check(f"gen:{client_ip}", max_requests=100, window_seconds=3600)
#     if not allowed:
...
#     try:
#         raw_context = gift.get("dua_context") or "{}"
#         context = json.loads(raw_context)
#     except Exception as e:
#         logger.exception("Failed to parse dua_context")
#         raise HTTPException(500, f"Invalid gift context: {str(e)}")
...
#         try:
#             logger.info(f"Sending gift email to {req.recipientEmail} for gift {req.giftId}")
...
# #             conn = db._get_conn()
# #             conn.execute("UPDATE gifts SET delivery_status = 'sent', delivery_method = 'email', recipient_contact = ? WHERE gift_id = ?",
# #                          (req.recipientEmail, req.giftId))
# #             conn.commit()
# #             conn.close()
# #             db.track("gifts_email")
...
#         conn = db._get_conn()
#         conn.execute("UPDATE gifts SET recipient_contact = ?, delivery_method = 'sms', stripe_session_id = ? WHERE gift_id = ?",
#                      (req.recipientPhone, session.id, req.giftId))
#         conn.commit()
#         conn.close()
...
#         conn = db._get_conn()
#         conn.execute("UPDATE gifts SET delivery_method = 'postcard', postcard_address_json = ?, stripe_session_id = ? WHERE gift_id = ?",
#                      (addr_json, session.id, req.giftId))
#         conn.commit()
#         conn.close()
#         return {"status": "checkout", "url": session.url}
...
#   <meta property="og:description" content="{sender} made a du'a for you during the blessed last nights of Ramadan."/>
#   <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;0,700;1,400;1,700&family=Amiri:wght@400;700&display=swap" rel="stylesheet"/>
#   <style>
#     *{{margin:0;padding:0;box-sizing:border-box;}}
#     body{{font-family:'Cormorant Garamond',serif;background:linear-gradient(160deg,#0a1628 0%,#112240 40%,#0d1f3c 70%,#091525 100%);color:#e8dcc8;min-height:100vh;padding:40px 20px;line-height:1.8;}}
...
#     <a href="/" class="cta">Make a du'a for someone you love</a>
...
def get_length_instruction(member_count: int) -> str:
    if member_count <= 2:
        return "Write a heartfelt du'a of approximately 2000-2500 words total. Include 1-2 Quranic verses per person with a rich opening and closing. Do not cut the du'a short — complete every section fully, including the family closing and Ameen."
    elif member_count <= 4:
        return "Write a comprehensive du'a of approximately 2500-3000 words total. Include 1-2 Quranic verses per person with full transliteration. Complete every section fully including the family closing and Ameen."
    elif member_count <= 6:
        return "Write a detailed du'a of approximately 3000-3500 words total. Include 1-2 Quranic verses per person with full transliteration. Complete every section fully including the family closing and Ameen."
    else:
        return "Write a thorough du'a of approximately 3500-4000 words total. Include 1-2 Quranic verses per person with full transliteration. Complete every section fully including the family closing and Ameen."

def get_max_tokens(member_count: int) -> int:
    if member_count <= 2: return 4096
...
async def startup():
    ...
    # Fix #7: No secrets in logs
    logger.info("=" * 50)
    logger.info("mydua.ai v1.1 — Production")
    logger.info("=" * 50)
    logger.info(f"AI Provider:  {AI_PROVIDER} ({ANTHROPIC_MODEL})")
    logger.info(f"Anthropic:    {'configured' if ANTHROPIC_API_KEY else 'NOT SET'}")
    logger.info(f"OpenAI:       {'configured' if OPENAI_API_KEY else 'not set'}")
...
@app.get("/api/health")
async def health_check():
    return {"status": "ok", "provider": AI_PROVIDER, "version": "1.1",
            "timestamp": datetime.now(timezone.utc).isoformat()}
...
@app.get("/support-thank-you", response_class=HTMLResponse)
async def support_thank_you():
    share_text = "I just supported mydua.ai — a free tool that writes personalized du'as for your family during the last 10 nights of Ramadan. Try it: " + APP_BASE_URL + "/?ref=share"
